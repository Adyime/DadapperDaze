...
export default function ProductDetail({
  ...
  const fetchColorImages = useCallback(
    async (color: string) => {
      if (!color) return;
      ...
      try {
        console.log(
          `Fetching images for product ${product.id}, color ${color}`
        );
        const response = await fetch(
          `/api/products/images?productId=${product.id}&color=${color}`
        );
        ...
        setColorImages(
          product.images.filter((img) => img.color === color || !img.color)
        );
      } finally {
    ...
    <div className="max-w-7xl mx-auto py-10 px-4 grid grid-cols-1 md:grid-cols-2 gap-y-12 gap-x-16 md:gap-y-0">
      ...
      <div className="flex flex-col gap-6 md:pr-8">
            ...
            {isLoadingImages ? (
              <CarouselItem>
                <div className="relative aspect-square rounded-2xl bg-drb-light flex items-center justify-center">
                  <span className="text-drb-gray font-sans">
                    Loading images...
                  </span>
                </div>
              </CarouselItem>
          ...
          <div className="flex space-x-3 overflow-x-auto py-2">
            {colorImages.map((image, index) => (
                ...
                className={`relative h-16 w-16 border-2 rounded-xl overflow-hidden flex-shrink-0 transition-all
                  ${
                    index === selectedImageIndex
                      ? "border-drb-pink ring-2 ring-drb-pink"
                      : "border-drb-light hover:border-drb-pink"
                  }
                `}
              ...
              >
                <Image
                  src={
                    image.imageUrl || "/placeholder.svg?height=600&width=600"
                  }
        ...
        <div className="flex flex-wrap gap-3 mt-2">
          <span className="block w-full text-sm font-sans text-drb-dark mb-1">
            Choose color:
          </span>
              ...
              >
                {item.image ? (
                  ...
                  <div className="absolute inset-0 bg-drb-light flex items-center justify-center rounded-full">
                    <span className="text-xs text-drb-gray font-sans">
                      {item.color}
                    </span>
                  </div>
                )}
      ...
      <div className="flex flex-col gap-8 md:pl-8">
        <div className="flex flex-col gap-2 border-b border-drb-light pb-4">
          <h1 className="font-heading text-4xl font-extrabold text-drb-dark mb-0 leading-tight">
            {product.name}
          </h1>
          <p className="text-drb-pink font-heading text-lg font-semibold">
        ...
        <div className="flex items-baseline gap-4">
          ...
          product.discountedPrice < product.price ? (
            <>
              <span className="text-3xl font-extrabold text-drb-pink font-heading">
                {formatPrice(product.discountedPrice)}
              </span>
              <span className="text-drb-gray line-through font-sans">
        ...
        {selectedColor && (
          <div className="mb-2">
            <p className="font-sans font-medium text-drb-dark">
              Color:{" "}
              <span className="text-drb-pink font-semibold">
                {selectedColor}
              </span>
            </p>
          </div>
        ...
        {availableSizes.length > 0 && (
          <div className="space-y-2">
            <span className="block text-sm font-sans text-drb-dark mb-1">
              Choose size:
            </span>
            <div className="flex flex-wrap gap-3">
              {availableSizes.map((size: string) => {
                const isSelected = size === selectedSize;
                const isAvailable = getVariantAvailability(
                  selectedColor!,
                  size
                );
                return (
                  <button
                    ...
                    className={`w-12 h-12 rounded-xl flex items-center justify-center border-2 font-heading text-lg font-bold transition-colors
                      ${
                        isSelected
                          ? "bg-drb-pink text-white border-drb-pink"
                          : "bg-white text-drb-dark border-drb-light"
                      }
                      ${
                        !isAvailable
                          ? "opacity-40 cursor-not-allowed"
                          : "hover:bg-drb-light hover:border-drb-pink"
                      }
                      focus:outline-none focus:ring-2 focus:ring-drb-pink
                    `}
        ...
        <p className="text-drb-gray font-sans text-base leading-relaxed mt-2 mb-4">
          {product.description}
        </p>
        <div className="pt-2">
