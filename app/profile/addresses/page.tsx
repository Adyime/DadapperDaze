      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {hasAddresses ? (
            ...
            <Card key={address.id} className="relative">
              ...
              <CardFooter className="flex gap-2 border-t pt-4">
                ...
                <form action={`/api/addresses/${address.id}/delete`} method="post">
                  <Button variant="outline" size="sm" className="flex items-center gap-1 text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700">
                    <Trash className="h-3.5 w-3.5" />
                    Delete
                  </Button>
                </form>
                {!address.isDefault && (
                  <form action={`/api/addresses/${address.id}/set-default`} method="post" className="ml-auto">
                    <Button variant="ghost" size="sm" className="flex items-center gap-1 text-green-600 hover:text-green-700 hover:bg-green-50">
          ...
          <Card className="bg-muted/50 border-dashed">
            <CardHeader>
              <CardTitle className="text-muted-foreground">No Saved Addresses</CardTitle>
              <CardDescription>
