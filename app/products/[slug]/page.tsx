  ...
  try {
    if (imageData && typeof imageData === "object") {
      ...
      if ("data" in imageData) {
        return `data:image/jpeg;base64,${Buffer.from(imageData.data).toString(
          "base64"
        )}`;
      }
...
export default async function ProductPage({
  ...
  const imagesByColor = product.images.reduce<Record<string, any[]>>(
    (acc, img) => {
  ...
  const processedImages = product.images.map((img) => {
    ...
    console.log(
      `Server: Processing image ${img.id}, color: ${img.color}, has URL: ${
