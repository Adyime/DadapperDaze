...
export const getProducts = cache(
  async ({
    ...
    const cacheKey = `products:${categoryId || "all"}:${minPrice || 0}:${maxPrice || "max"}:${sort || "default"}:${page}:${limit}`
    ...
    const variantProducts = baseProducts.flatMap(product => {
      ...
      return uniqueColors.map(color => {
        ...
        const colorImage = product.images.find(img => img.color === color) || product.images[0] || null;
...
export const getSimpleProducts = cache(
  async ({
    ...
    const cacheKey = `simple-products:${categoryId || "all"}:${minPrice || 0}:${maxPrice || "max"}:${sort || "default"}:${page}:${limit}`
    ...
    const processedProducts = products.map(product => {
      console.log(`Processing product: ${product.name}, Image count: ${product.images.length}`);
      ...
      const processedImages = product.images.map(img => {
        ...
        try {
          console.log(`Processing image ${img.id}, type: ${typeof img.image}, color: ${img.color || 'none'}`);
          
          // Check if image is already a string (might be a data URL)
          if (typeof img.image === 'string') {
            console.log(`Image is already a string, length: ${(img.image as string).length}`);
